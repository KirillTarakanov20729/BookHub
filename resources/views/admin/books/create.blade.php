        ...
        <div class="row justify-content-center">
            <div class="col-lg-4 col-md-9 col-12">
                <form action="{{ route('admin.books.store') }}" method="POST">
                    ...
                    <div class="mt-3">
                        <label for="description" class="fs-4">{{__("Короткое описание")}}</label>
                        <input id="description" name="description" type="text" class="form-control fs-5" value="{{request()->old('last_name')}}"/>
                        @error('description')
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
